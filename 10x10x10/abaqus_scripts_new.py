...
def abaqus_merge2(m, rt):
    SingleInstances_List = rt.instances.keys()

    SingleInstances_Num = len(SingleInstances_List)

    rt.InstanceFromBooleanMerge(name='Merge-0', instances=([rt.instances[SingleInstances_List[i]]

                                                            for i in range(0, 2)]), keepIntersections=ON,

                                domain=GEOMETRY, originalInstances=SUPPRESS)

    for i in range(1, SingleInstances_Num - 2):
        rt.InstanceFromBooleanMerge(name='Merge-%d' % (i), instances=([rt.instances[SingleInstances_List[i + 1]],

                                                                       rt.instances['Merge-%d-1' % (i - 1)]]),
                                    keepIntersections=ON,

                                    domain=GEOMETRY, originalInstances=SUPPRESS)

        del m.parts['Merge-%d' % (i - 1)]

    rt.InstanceFromBooleanMerge(name='Merge-1', instances=([rt.instances[SingleInstances_List[SingleInstances_Num - 1]],

                                                            rt.instances['Merge-%d-1' % (SingleInstances_Num - 3)]]),
                                keepIntersections=ON,

                                domain=GEOMETRY, originalInstances=SUPPRESS)

    del m.parts['Merge-%d' % (SingleInstances_Num - 3)]
...
def abaqus_historyOutput(w, q):
    ...
    for file_name, arr in zip(file_names, arrs):
        file_name = '{}_{}.csv'.format(file_name, w)
        print('x')
        array_to_csv(path=file_name, arr=arr.transpose(), dtype=np.float32, mode='a')
        print('y')
...
def control_abaqus(w, offspring):
    for q in range(ini_pop, end_pop + 1):
        m = mdb.models['Model-%d' % q]
        rt = m.rootAssembly
        # save_log('[{}][Gen{} offspring{}] Model and root assembly set!'.format(now_s(), w, q))
        abaqus_cad(offspring, m, rt, q)
        # save_log('[{}][Gen{} offspring{}] Cad complete!'.format(now_s(), w, q))
        abaqus_merge4(5, m, rt)
        # save_log('[{}][Gen{} offspring{}] Merge complete!'.format(now_s(), w, q))
        abaqus_mesh(m)
        # save_log('[{}][Gen{} offspring{}] Mesh complete!'.format(now_s(), w, q))
        abaqus_material_anisotropic(offspring, 1500, 1200, 1500, 0.35, 0.35, 0.35, 450, 550, 450, m, q)
        # save_log('[{}][Gen{} offspring{}] Material set complete!'.format(now_s(), w, q))
        abaqus_step(m, rt)
        # save_log('[{}][Gen{} offspring{}] Step set complete!'.format(now_s(), w, q))
        abaqus_BC(offspring, m, rt, q)
        # save_log('[{}][Gen{} offspring{}] Boundary condition set complete!'.format(now_s(), w, q))
        abaqus_jobsetting(w, q)
        # save_log('[{}][Gen{} offspring{}] Job setting complete!'.format(now_s(), w, q))
        abaqus_jobsubmit(w, q)
        # save_log('[{}][Gen{} offspring{}] Job submit complete!'.format(now_s(), w, q))
        ...
        save_log('[{}][Gen{} offspring{}] Complete'.format(now_s(), w, q))
    mdb.Model(modelType=STANDARD_EXPLICIT, name='Model-1')
    del mdb.models['Model-%d' % (end_pop + 1)]
    save_log('[{}][Gen{}] All current generation work done!'.format(now_s(), w))
    os.remove('./args')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if os.path.isfile('./args'):
            ...
            control_abaqus(w=w, offspring=offspring)

        elif os.path.isfile('./args_end'):
            os.remove('./args_end')
            break
        sleep(1)
    save_log('ABAQUS SESSION DONE!')
