...
## Path options
setPath = r'C:\Users\dcas\PythonCodes\Coop\pythoncode\10x10x10'  # The folder path of abaqus_scripts.py & csv files
...
abaqus_execution_mode = 'noGUI'  # 'noGUI' for cui mode or 'script' for gui mode
                ...
                for j in range(ly):
                    for k in range(lz):
                        ...
                        if topos[a][k][j][i] == 1:
                            for x in range(2):
                                for y in range(2):
                                    if k + direc[x] in range(lz) and j + direc[y] in range(
                                            ly):  # correction: a,b,c >> lx, ly, lz
                                    ...
                                    for z in range(2):
                                        if k + direc[x] in range(lz) and j + direc[y] in range(ly) and i + direc[
                                            ...
                                            if topos[a][k + direc[x]][j + direc[y]][i + direc[z]] == 1:
                                                if topos[a][k + direc[x]][j][i] == 0 and topos[a][k][j + direc[y]][
                                                    i] == 0 and topos[a][k][j][i + direc[z]] == 0:
                                                    if topos[a][k + direc[x]][j + direc[y]][i] == 0 and \
                                                            topos[a][k + direc[x]][j][i + direc[z]] == 0 and \
                                                            topos[a][k][j + direc[y]][i + direc[z]] == 0:
                                                        topos[a][k][j][i] = 0
                                                        topcrs = 1
                                                        flag = 1
                                                        break
    ...
    if w == 1:
        ...
        for j in range(1, w):
            ...
            for i in range(len(parents_topo)):
                if np.array_equal(parents_topo[i], topologys[0]) == True or np.array_equal(parents_topo[i],
                                                                                           topologys[1]) == True:
                    flag1 = 1
                    break
                else:
    ...
    while len(offspring) < end_pop:  # end_pop = populations
        ...
        while candiat == 0:  ## finding pair of offspring chromosomes
            ...
            for j in range(len(candidates_reslt)):
                ...
                if len(y0_num[0]) == 0:
                    continue
                # with open('topo', mode='wb') as f:
                #     pickle.dump(offsprings_mmm[0], f)
                # with open('topoend', mode='wb') as f:
                #     pickle.dump(topoend, f)
                # exit()
                offsprings_mmm0, topoend = bfs_total(offsprings_mmm[0], lx, ly, lz, topoend=topoend)
                numberings += 1
                ...
                if len(y0_num[0]) == 0:
                    continue
                offsprings_mmm1, topoend = bfs_total(offsprings_mmm[1], lx, ly, lz, topoend=topoend)
                numberings += 1
                print('bfs:', numberings)
                if topoend == 0:
                    continue
...
def generate_random(q):
    ...
    while topoend:
        ...
        topo_1 = design_const_one(topo, lx, ly, lz)
        flag = 1
        while flag:
            for x in range(lx):
                for y in range(ly):
...
#     fitness_values = evaluation2(topo_p, reslt_p)      #input variable unmatched
...
#     print("    Job       objective 1     objective 2")
#     print("     |             |               |")
#     for i in range(len(index)):
#         print("  Job-%d-%d   [%E   %E]" % (w, i+1, fitness_values[i, 0], fitness_values[i, 1]))
#     plt.scatter(fitness_values[:, 0], fitness_values[:, 1])  # plt: not defined >> matplotlib.pyplot; plt Error
