...
class App(threading.Thread):
    ...
    def onclick_submit_btn(self):
        ...
        self.submit_btn.config(background='#00FF00', text='저장 완료')
        with open('./PARAMS_MAIN', mode='wb') as f:
            dump_dict = {**{'setPath': self.setPath.get()}, **params_main}
            pickle.dump(dump_dict, f)
            print('Dumping complete:\n', dump_dict)

    def onclick_exit_btn(self):
        self.message = self.setPath.get()
        print('Message set: ', self.message)
    ...
    def show_parameters(self, loaded):
        ...
        self.exit_btn = tk.Button(self.right_frame, width=button_width, text='GA 실행',
                             command=self.onclick_exit_btn)
