...
def inspect_clone_in_previous_generations(topology, end_pop, w, lx, ly, lz):  # inspect topology clones
    if w == 1:
        ...
        for generation_idx in range(1, w):
            parents_topologies = np.genfromtxt('topo_parent_' + str(generation_idx) + '.csv', delimiter=',', dtype=int).reshape((end_pop, lx * ly * lz))
...
def crossover(chromosome_1, chromosome_2, cutting_section,
    ...
    offspring1 = np.zeros(topologies.shape[1])
    offspring2 = np.zeros(topologies.shape[1])
...
if __name__ == '__main__':
    path = r'C:\Users\dcas\PythonCodes\Coop\pythoncode\10x10x10 - 복사본'
    os.chdir(path)
    ...
    generate_offspring(topologies=topologies, w=1, end_pop=end_pop,
                       mutation_rate=0.05, add_probability=0.01, timeout=0.5, lx=lx, ly=ly, lz=lz)
