...
import pandas as pd
import threading
import GraphicUserInterface2 as GUI
from GeneticAlgorithm import generate_offspring, array_to_csv
from PostProcessing import *
import asyncio


setPath = r'C:\Users\dcas\Desktop\temp'
...
def parent_export(w, results, results_1, topologies, next_generations):
    ...
    if mode == 'GA':
        if restart_pop == 0:
            for w in range(ini_gen, end_gen + 1):
                ...
                pop, next_generations = selection(np.append(topologies, topologies_1, axis=0), fitness_values, end_pop)  # selection (index)
                # save_variable_for_debugging(debug_code=3, w=w, debug_variable=[pop, next_generations])

                parent_export(w, results, results_1, topologies, next_generations)
                print('iteration:', w)
            ...
            for w in range(ini_gen + 1, end_gen + 1):
                ...
                parent_export(w, results, results_1, topologies, next_generations)
                print('iteration:', w)
            visualize(w, restart_pop, lx, ly, lz, end_gen, penalty_coefficient, evaluation_version, MaxRF22, setPath, end_pop)
...
async def main():
    ...
    task1 = asyncio.create_task(job1(restart_pop=restart_pop))
    print(1)
