                ...
                for k in range(lz):
                    if topo[k, j, i] == 1:
                        for m in range(4):
                            ...
                            for z in [1, -1]:
                                if k + direction[m, 0] in range(lz) and j + direction[m, 1] in range(ly) and i + z in range(lx):
                                            ...
                                            if topo[k + direction[m, 0], j + direction[m, 1], i] == 0 and topo[k + direction[m, 0], j, i + z] == 0 and topo[k, j + direction[m, 1], i + z] == 0:
                                                ...
                                                if rand1 == 3:
                                                    ...
                                                    if rand2 == 1:
                                                        ...
                                                        topo[k, j + direction[m, 1], i + z] = 1
                                                flag = 1
    return topo, np.array_equal(topologies, topo)
...
def one_connected_tree(arr_3d, add_probability):
    ...
    return arr_copy, is_no_change, add_random_voxels_count
    ...
    while True:
        ...
        while not is_timeout:
            ...
            if total_random_voxel_additions > mutation_probability * lx * ly * lz:
                ...
                is_timeout = True
                break
            arr_3d_mutated_copy, not_changed1, random_voxel_additions = one_connected_tree(
...
if __name__ == '__main__':
    ...
    for idd in range(100):
        rand_arr = random_array(shape=(10, 10, 10), probability=0.25)
        print(f'<<<<< Offspring {idd + 1} >>>>>')
        arr = mutate_and_validate_topology(rand_arr, mutation_probability=0.1, add_probability=0.01, timeout=0.5)
        volume_frac = np.count_nonzero(arr)/10
        total_volume_frac += volume_frac
        print(f'Volume fraction: {volume_frac:.1f} %\n')
        # visualize_one_cube(arr, full=False)
    print(f'Average volume fraction: {total_volume_frac/100:.1f} %')
