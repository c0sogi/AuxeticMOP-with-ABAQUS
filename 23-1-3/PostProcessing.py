...
def hypervolume_calculation(fv1_sort, fv2_sort, w, restart_pop, lx, ly, lz, evaluation_version,
                            penalty_coefficient, max_rf22):
    fit_max, fit_min = max_fitval(w=w, restart_pop=restart_pop, lx=lx, ly=ly, lz=lz,
...
def visualize(w, restart_pop, lx, ly, lz, penalty_coefficient, evaluation_version, max_rf22, directory,
              end_pop, parent_conn):
    ...
    if os.path.isfile(f'Plot_data_{w}'):
        with open(f'Plot_data_{w}', mode='rb') as f_read:
            read_data = pickle.load(f_read)
        with open(f'Plot_data_{w}', mode='wb') as f_write:
