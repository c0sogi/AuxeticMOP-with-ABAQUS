...
from GeneticAlgorithm import generate_offspring, array_to_csv, parent_import
from PostProcessing import pareto_front_finding, evaluation, visualize
...
def load_pickle_and_set_parameters(file_name):
    with open(file=file_name, mode='rb') as f_read:
        read = Parameters(**pickle.load(f_read))
        print(f'[MAIN] Loading Parameters Complete')
        for key, value in asdict(read).items():
            print(f'- {key}: {value}')
        return read
...
def one_generation(w, restart, p_conn, params):
    if restart:
        ...
        offspring = generate_offspring(topologies=topologies, w=w, end_pop=params.end_pop,
                                       ...
                                       add_probability=params.add_probability,
